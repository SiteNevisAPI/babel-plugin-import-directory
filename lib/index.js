...
var toCamelCase = function toCamelCase(name) {
  return name.replace(/([-_.]\w)/g, function (_, $1) {
  ...
  return {
    visitor: {
      ImportDeclaration: function ImportDeclaration(path, state) {
        ...
        var nameTransform = state.opts.snakeCase ? toSnakeCase : toCamelCase;

        var _files = getFiles(checkPath, state.opts.exts, [], isRecursive);

        var files = _files.map(function (file) {
          return [file, nameTransform(state.opts.withPath ? file.join("_") : file[file.length - 1]), path.scope.generateUidIdentifier(state.opts.withPath ? file.join("_") : file[file.length - 1])];
        });
